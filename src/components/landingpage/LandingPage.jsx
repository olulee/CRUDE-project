  ...
  let sampleData = [
    {
      ...
      title: 'Software Dev',
      description:
        'I have over 4 year experience as frontend developer. Using react, I have over 3 years experience. ',
      status: 'Available',
  ...
  return (
    <div className='land_wrapper'>
      <div className='user_container'>
        Please
        <CSVLink data={sampleData}>click here</CSVLink> to download a CSV sample
        document for bulk upload
      </div>

      {users.length === 0 ? (
        <div className='no_itme_wrapper'>
          <div>User list empty, please click create user, to add user</div>
        </div>
      ) : (
        <div className='user_container'>
          <div className='user_page_top'>
            ...
            <button>
              <CSVLink data={users}>Download as CSV</CSVLink>
            </button>
          </div>
            ...
            {users.map((each, id) => {
              return (
                <div className='card' key={id}>
                  <div
                    className={
                      each.status === 'Available' ? 'status' : 'status_not'
                    }
                  >
                    {each.status}
                  </div>
